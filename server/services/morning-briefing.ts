...
export async function generateMorningBriefing(customerId: string) {
  ...
  if (weather && !("error" in weather)) {
    ...
    for (const day of forecast) {
      if (parseInt(day.chanceOfRain) > 60) {
        suggestions.push(`🌧️ Rain expected ${day.date} — consider gutter cleaning`);
      }
      if (parseInt(day.chanceOfSnow) > 40) {
        suggestions.push(`❄️ Snow expected ${day.date} — schedule driveway treatment`);
      }
  ...
  return {
    greeting: `Good morning${customer.first_name ? `, ${customer.first_name}` : ""}! ☀️`,
    weather,
