...
async function sendProWarning(jobId: string, proId: string, minutesRemaining: number): Promise<void> {
  try {
    ...
    if (profile[0]?.phone) {
      await sendSms({
        to: profile[0].phone,
        message: `⚠️ UpTend: You have ${minutesRemaining} minutes to check in for job #${jobId.slice(-6)}. Open the app and tap "I'm Here" or you may be reassigned.`,
      });
    }
...
async function flagForAdminReview(jobId: string, proId: string, reason: string): Promise<void> {
  try {
    ...
    await Promise.all([
      sendSms({
        to: ADMIN_PHONE,
        message: `⚠️ ADMIN: Job #${jobId.slice(-6)} - Pro delayed but communicated. Reason: "${reason}". Needs review.`,
      }),
      sendEmail({
        to: ADMIN_EMAIL,
