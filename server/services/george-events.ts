...
const INTEL_QUESTIONS: Record<string, string> = {
 ...
 moving: "Quick question - is the pickup and drop-off ground floor, or are there multiple levels? Helps us send the right crew size.",
 truck_unloading: "Quick question - is there elevator access at the destination, or will the crew need to navigate multiple floors?",
...
export async function sendPostServiceFollowUp(): Promise<{ sent: number; skipped: number }> {
 ...
 const message = `Hey ${name}! George here How's everything looking after your ${service}? If anything needs attention, just reply and I'll take care of it.`;
...
export async function sendWeatherHeadsUp(): Promise<{ sent: number; skipped: number; alertCount: number }> {
 ...
 const message = `Hey ${name}, George here. ${eventName} headed our way. ${tip} Need help prepping? Just reply.`;
...
export async function scanGovernmentContracts(): Promise<{ found: number; emailed: boolean }> {
 ...
 <h1 style="color:white;margin:0;font-size:20px"> Government Contract Alert</h1>
 ...
 <p style="font-size:11px;color:#999;margin-top:12px">This is an automated scan from George. Runs daily at 7 AM EST.</p>
 </div>
 </div>
 `;
