...
interface AfterPhotosCaptureProps {
  jobId: string;
  serviceType: string;
  onComplete: () => void;
  onCancel: () => void;
...
export function AfterPhotosCapture({ jobId, onComplete, onCancel }: AfterPhotosCaptureProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiRequest("POST", `/api/jobs/${jobId}/verification/after-photos`, {
        photos,
        gpsCoordinates,
      });
    },
    onSuccess: () => {
      toast({
        ...
        description: "You can now generate the sustainability report",
      });
      onComplete();
    },
    onError: (error: any) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Document the job site after completing work
              </p>
            </div>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1 ml-4 list-disc">
            <li>Take at least 2 photos showing the cleaned area</li>
            <li>Show that all items have been removed</li>
            <li>Document the final condition</li>
            <li>Ensure the area is clean and tidy</li>
          </ul>
        </div>
        ...
        <div className="space-y-3">
          ...
          {!gpsCoordinates && (
            <p className="text-xs text-muted-foreground">
              GPS coordinates verify job completion location
            </p>
          )}
        </div>
