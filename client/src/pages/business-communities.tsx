              ...
              {demoCommunities.map(c => (
                <Card key={c.id} className="p-5 hover:shadow-md transition-shadow">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div><span className="text-muted-foreground">Units:</span> <span className="font-medium">{c.units}</span></div>
                    <div><span className="text-muted-foreground">Dues:</span> <span className="font-medium">${c.monthlyDues}/mo</span></div>
                ...
                <TableBody>
                  {demoViolations.map(v => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell>{new Date(v.deadline).toLocaleDateString()}</TableCell>
              ...
              {demoApprovals.map(a => (
                <Card key={a.id} className="p-5">
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    ...
                    <span className="flex items-center gap-1"><Calendar className="w-4 h-4 text-muted-foreground" /> Deadline: {new Date(a.deadline).toLocaleDateString()}</span>
                  </div>
                ...
                <TableBody>
                  {demoCalendar.map(s => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell>{new Date(s.nextDate).toLocaleDateString()}</TableCell>
